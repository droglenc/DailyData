...
```{r}
tbltmp <- xtabs(~sleep,data=df)
```

::: {.column style="width: 40%; font-size: 60%;"}
- Hours of sleep is number of overnight hours from waking up that morning (i.e., no lag).
- Very few days had less than 6 (n=`r cumsum(tbltmp)[["5"]]` d) or more than 9 (n=`r FSA::rcumsum(tbltmp)[["10"]]` d) hours of sleep. Thus, these were combined with 6 and 9 hours to form "&leq;6 h" and "&geq;9 h" categories.  
:::
...
::: {.column style="width: 40%; font-size: 60%;"}
- To allow for a chi-square test of association (i.e., for sample size reasons) the "Tough, but things done", "Tough, but getting through", and "Tears, hanging on" levels were combined into one level called "Tough." 
